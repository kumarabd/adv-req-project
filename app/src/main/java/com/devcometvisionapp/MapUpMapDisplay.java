 ...
 */
public class MapUpMapDisplay extends com.devcometvisionapp.MapDisplay {
  ...
  public void setMap(GroundOverlay newMap) throws MapImageTooLargeException {
    ...
    try {
      ...
      Log.w(StartNavigation.LOG_TAG, "Failed to load map image for " + newMap.getName(), ex);
      mapImage = null;
    ...
    if (mapImage == null) {
      if (newMap != null) {
        Log.w(StartNavigation.LOG_TAG, "Map image failed to load, map not set.");
      }
      spotSet = false;
      mapData = null;
      return;
