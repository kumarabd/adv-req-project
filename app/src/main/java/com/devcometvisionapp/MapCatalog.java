...
import com.devcometvisionapp.kml.GroundOverlay;
import com.devcometvisionapp.kml.KmlFeature;
import com.devcometvisionapp.kml.KmlFile;
import com.devcometvisionapp.kml.KmlFolder;
import com.devcometvisionapp.kml.KmlInfo;
import com.devcometvisionapp.kml.KmlParser;
import com.devcometvisionapp.kml.KmzFile;
import com.devcometvisionapp.kml.Placemark;
...
public class MapCatalog {
  ...
  /**
   * Loads a named map from a KML or KMZ file. If name is not provided, returns any one map stored
   * in the file.
   *
   * @param kmlInfo KML or KMZ file containing the map
   * @param mapName the map to be read, or 'null' for any map
   * @return KmlFolder containing one GroundOverlay and possibly multiple Placemarks, or 'null' if
  ...
  public static KmlFolder loadMap(KmlInfo kmlInfo, String mapName) {
    ...
    try {
      ...
      Log.w(StartNavigation.LOG_TAG, "Failed to parse KML file: " + kmlInfo.toString(), ex);
    }
  ...
  /**
   * Returns a KmlFolder that contains only a single GroundOverlay (map) with optionally multiple
   * Placemarks as well. Note that if the KmlFolder was inside a Document tag, the icons stored at
   * Document level should be added separately to the KmlFolder returned from this method.
   *
   ...
   * @return a newly created KmlFolder containing only a single GroundOverlay and its associated
   *     Placemarks
   */
  ...
  private List<KmlFolder> inMaps = new ArrayList<>();
  ...
  /**
   * Adds a known created filename to catalog since some versions of Android have issues refreshing
   * map catalog after new maps are created.
   *
  ...
  /**
   * Parses the given KML/KMZ file and returns a map from there. This method is intended to be used
   * when a user opens a KML attachment in mail, or uses a file manager to open such file. The map
   * is returned as a KmlFolder containing exactly one GroundOverlay and optionally multiple
   * Placemarks.
   *
   * @param mapFile containing at least one GroundOverlay definition
   * @return KmlFolder containing only a single GroundOverlay (first found in the file) and
   *     optionally multiple Placemarks, or {@code null} if no GroundOverlays were found in the
   *     file. The map is the first item stored in the returned KmlFolder features.
   */
   ...
   * @return {@code true} if this catalog contains another map stored in the same KML or KMZ file
   */
  ...
  /**
   * Finds all maps that contain a location. This is slightly faster than calling
   * groupMapsByDistance(longitude, latitude) followed by getLocalMaps(). Result is the same
   * though.
   *
  ...
  public Iterable<KmlFolder> getMapsContainingPoint(float longitude, float latitude) {
    List<KmlFolder> result = new ArrayList<>();
  ...
  /**
   * Groups maps by distance from the given point. Maps will be divided to three groups: maps
   * containing the point, maps near the point (< 50km), maps far from point.
   *
   * @param longitude of the location
   * @param latitude of the location
   */
  ...
  /**
   * Quickly check if a map is certainly farther away than given distance. Even if this function
   * returns false, it is still possible that the map is farther away than the given distance. This
   * function only quickly determines maps that certainly are farther away.
   *
   ...
   * @return true if the map is at least given distance away from the closest point of the map.
   */
  private boolean mapIsFartherAway(double distanceM, GroundOverlay map, double lat, double lon) {
    double latMin = 90;
    double latMax = -90;
    double lonMin = 180;
    double lonMax = -180;
   ...
   * Note: Call groupMapsByDistance(longitude, latitude) first.
   *
   * @return Iterable<KmlFolder> of maps within 50 km (30 mi) of the location
   */
   ...
   * Note: Call groupMapsByDistance(longitude, latitude) first.
   *
   * @return Iterable<KmlFolder> of maps farther than 50 km (30 mi) of the location
   */
  ...
  private Collection<KmlFolder> parseMapsFrom(KmlInfo kmlInfo, KmlParser parser) {
    ...
    List<KmlFolder> maps = new ArrayList<>();
    List<Placemark> sharedPlacemarks = new ArrayList<>();
    try {
      Iterable<KmlFeature> features = parser.readFile(kmlInfo.getKmlReader());
      for (KmlFeature feature : features) {
  ...
  private Iterable<KmlInfo> findKmlData() {
    List<KmlInfo> kmlData = new ArrayList<>();
   ...
   * @return Collection<KmzFile> containing all kml entries in the kmz file. Returns an empty list
  ...
  private Collection<KmzFile> scanKmz(File file) {
    ...
    try {
      ...
      Log.w(StartNavigation.LOG_TAG, "Not a valid KMZ file: " + file.getName(), ex);
      return Collections.emptyList();
    }
    List<KmzFile> kmlEntries = new ArrayList<>();
   ...
   */
  private Comparator<KmlFolder> mapSorter = (folder1, folder2) -> {
    GroundOverlay m1 = folder1.getFirstMap();
    GroundOverlay m2 = folder2.getFirstMap();
    if (m1 == null) {
      return (m2 == null ? 0 : 1);
    } else if (m2 == null) {
      return -1;
