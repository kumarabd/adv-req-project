...
import com.devcometvisionapp.kml.GroundOverlay.Tiepoint;
...
public class GeoToImageConverter {
  ...
  private void initGeoToImageMatrixRotated() {
    ...
    if (!success) {
      Log.w(StartNavigation.LOG_TAG, "FAILED to initialize geoToImageMatrix from rotation");
    }
  }
  ...
  private void initGeoToImageMatrixTiePoints() {
    ...
    if (!geoToImageMatrix.setPolyToPoly(geoPoints, 0, imagePoints, 0, 4)) {
      Log.w(StartNavigation.LOG_TAG, "FAILED to initialize geoToImageMatrix from tie points");
    }
  ...
  private void improveAccuracy() {
    ...
    for (Tiepoint pt : mapData.getTiepoints()) {
      ...
      LatLng geo = pt.getGeoPoint();
      location[0] = (float) geo.longitude;
      location[1] = (float) geo.latitude;
