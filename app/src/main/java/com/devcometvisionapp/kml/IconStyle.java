 ...
 */
package com.devcometvisionapp.kml;
...
import com.devcometvisionapp.ImageDiskCache;
...
public class IconStyle extends KmlData {
  ...
  private void readIconSize() {
    ...
    try {
      ...
      Log.e(StartNavigation.LOG_TAG, "Failed to read icon size: " + iconPath, ex);
    } finally {
  ...
  private Bitmap loadIcon() {
    ...
    try {
      ...
      Log.e(StartNavigation.LOG_TAG, "Failed to load icon: " + iconPath, ex);
      return null;
    } catch (MapImageTooLargeException ex) {
      Log.e(StartNavigation.LOG_TAG, "Icon too large: " + iconPath, ex);
      return null;
    } finally {
      FileUtil.tryToClose(in);
    }
