...
public class MapApiKeys {
  ...
  public static boolean isReleasedVersion(Context context) {
    ...
    if (signatureHash == RELEASE_KEY_SIGNATURE_HASH) {
      Log.i(StartNavigation.LOG_TAG, "This is an official release version of this app");
    } else {
      Log.i(StartNavigation.LOG_TAG, "This is not an official release version of this app");
    }
  ...
  private static int getSignatureHash(Context context) {
    ...
    try {
      ...
      if (info.signatures.length > 1) {
        Log.e(StartNavigation.LOG_TAG, "Found too many signatures: " + info.signatures.length);
        return 0;
      }
      int signatureHash = info.signatures[0].hashCode();
      Log.i(StartNavigation.LOG_TAG, String.format("Signature hash: 0x%X", signatureHash));
      return signatureHash;
    } catch (NameNotFoundException ex) {
      Log.e(StartNavigation.LOG_TAG, "Context package name was invalid", ex);
    }
    return 0;
  }
}
