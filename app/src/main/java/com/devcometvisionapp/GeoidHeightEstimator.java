...
public class GeoidHeightEstimator {
  ...
  private static Map<GeoIndex, Short> heightCache = new ConcurrentHashMap<>();
  ...
  private static Short[] getHeightsAt(double latitude, double longitude) {
    ...
    if (needsData) {
      ...
      try {
        ...
        Log.e(StartNavigation.LOG_TAG, "Failed to read geoid height data", ex);
  ...
  private static void readValues(Short[] results, GeoIndex sw, GeoIndex nw, GeoIndex ne, GeoIndex se)
    ...
    try (DataInputStream in = new DataInputStream(
        assets.open(HEIGHT_DATA, AssetManager.ACCESS_BUFFER))) {
      position = 0;
      if (nw.longitude != 359) {
