...
public class KmzDownloader extends Activity {
  private static final String PREFIX = "com.devcometvisionapp";
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (mapUrl == null) {
      Log.e(com.devcometvisionapp.StartNavigation.LOG_TAG, "KmzDownloader invoked with an invalid URL: " + urlString);
      cancelActivity(null);
      return;
    }
    ...
    progressDetails.setText(linguist.getString(R.string.progress_format, 0));
  ...
  private void updateProgress(int bytesDone, int totalSize) {
    ...
    if (totalSize != UNKNOWN_SIZE) {
      ...
      progressDetails.setText(linguist.getString(R.string.progress_format, bytesDone / 1024));
    }
  ...
  private void cancelDownload() {
    ...
    cancelActivity(linguist.getString(R.string.download_canceled));
  }
  ...
  private void returnActivityResult(File localFile) {
    Toast.makeText(this, linguist.getString(R.string.download_successful), Toast.LENGTH_SHORT)
        .show();
  ...
  private String createLocalPath(URL mapUrl) {
    ...
    File localDir = com.devcometvisionapp.FileUtil.getInternalMapDirectory();
    if (!localDir.exists()) {
  ...
  private void prepareUI() {
    ...
    progressDisplay = findViewById(R.id.progressView);
  ...
  private class DownloadFile extends AsyncTask<Void, Integer, File> {
    ...
    protected File doInBackground(Void... params) {
      ...
      try {
        ...
        Log.e(com.devcometvisionapp.StartNavigation.LOG_TAG,
              String.format("Map download failed from %s to %s", mapUrl, fileName), ex);
        return null;
      } finally {
        // All done, close open streams
        if (conn != null) {
          com.devcometvisionapp.FileUtil.tryToClose(in);
          conn.disconnect();
        }
        com.devcometvisionapp.FileUtil.tryToClose(out);
      }
    ...
    protected void onPostExecute(File mapFile) {
      if (mapFile != null) {
        ...
        cancelActivity(linguist.getString(R.string.download_failed, mapUrl));
      }
    }
